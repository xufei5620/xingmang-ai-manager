...
const api: AuthApi = {
  ...
  // totp：当前账号开了两步验证。111111 当输错，totpExpired 模拟超过 5 分钟。
  submitTwoFactor: async (code) => { record('two-factor', code); await waitForRelease('two-factor'); if (query.has('totpExpired')) throw new Error('等太久了，请重新输入密码登录'); if (code === '111111') throw new Error('验证码不对或已过期，请看验证器里最新的数字再试'); return { account: { userId: 7, username: 'totp-user', quota: 0, usedQuota: 0, group: 'default', role: 1 }, accessExpiresAt: null, siteId: 'solov' } },
...
function Fixture() {
  ...
  if (scenario === 'guide') return guideVisible ? <StartGuide platform={query.get('os') === 'mac' ? 'mac' : query.get('os') === 'linux' ? 'linux' : 'win'} tools={tools} signedIn={signedIn} resumeKey={query.has('resume') ? resumeScope : undefined} onDetect={async () => { record('detect'); await waitForRelease('detect') }} onInstall={async (route, version) => { record('install', version ? { route, version } : route); if (query.has('installFail') && calls.filter((item) => item.method === 'install').length === 1) throw new Error('Node.js 运行环境没装上，Gemini CLI 还没开始安装。下载 Node.js 时 ETIMEDOUT'); update(route, { installed: true, ...(version ? { version, update: null } : {}), ...(query.has('auto') ? { runtimeReady: true, pythonReady: true } : {}) }) }} onInstallRuntime={async () => { record('runtime'); setTools((items) => items.map((item) => ({ ...item, runtimeReady: true }))) }} onInstallPython={async () => { record('python'); await waitForRelease('python'); setTools((items) => items.map((item) => ({ ...item, pythonReady: true }))) }} onConfigure={async (route) => { record('configure', route); update(route, { configured: true, source: 'account', officialLoginRequired: false }) }} onLogin={() => { record('guide-login'); setSignedIn(true) }} onLaunch={async (route) => { record('launch', route); if (query.has('launchFail') && calls.filter((item) => item.method === 'launch').length === 1) throw new Error('请先确认账号连接，再打开工具。'); return query.has('launchCancel') ? false : undefined }} onComplete={(route) => record('complete', route)} onBack={() => { record('back'); setGuideVisible(false) }} onHelp={() => record('help')} {...(query.has('switchable') ? { accountName: 'peaker', onFailureAction: (action: string) => record('exit', action), onSwitchAccount: async (route: Exclude<GuideRoute, 'chat'>) => { record('switch', route); if (query.has('switchFail')) throw new Error('改用当前账号没有完成：分组不存在、不可用或名称重复。已恢复到切换前的配置。'); update(route, { source: 'account', configured: true, keyState: undefined }); return { provider: route === 'codexDesktop' ? 'codex' as const : route, target: 'account' as const, backupId: 'fixture-backup', verified: true, loginRequired: false, message: '已改用当前账号，连接自检通过。' } } } : {})} /> : <p>引导已暂停</p>
