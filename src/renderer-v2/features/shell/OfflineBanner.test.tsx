...
function withStatus(status: Partial<OnlineStatus>, child: Parameters<typeof createElement>[0], props: object = {}): string {
  return renderToStaticMarkup(createElement(OnlineStatusContext.Provider, { value: { offline: false, checking: false, recheck() {}, ...status } }, createElement(child, props)))
}
...
describe('renderer-v2 offline banner', () => {
  it('stays out of the page while the network works', () => {
    ...
    expect(withStatus({ offline: false }, OfflineBanner)).toBe('')
  })

  it('explains what still works and offers a recheck when offline', () => {
    const markup = withStatus({ offline: true }, OfflineBanner)
  ...
  it('sits at the top of the shell and relabels the balance failure', () => {
    const offline = withStatus({ offline: true }, Shell, shellProps())
    ...
    const online = withStatus({ offline: false }, Shell, shellProps())
