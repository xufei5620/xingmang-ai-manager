...
export function ListRow({ icon: Icon, leading, title, badge, desc, descMono, meta, actions, off, testId }: BaseProps & { icon?: Icon; leading?: ReactNode; title: ReactNode; badge?: ReactNode; desc?: ReactNode; descMono?: boolean; meta?: ReactNode; actions?: ReactNode; off?: boolean }) {
  return <div className={cx('xm-list-row', off && 'is-off')} data-testid={testId}>{leading ? <span className="xm-row-icon">{leading}</span> : Icon && <span className="xm-row-icon"><Icon size={18} aria-hidden="true" /></span>}<div className="xm-row-main"><div className="xm-row-title">{title}{badge}</div>{desc && <div className={cx('xm-row-desc', descMono && 'xm-mono')}>{desc}</div>}</div>{meta && <div className="xm-row-meta">{meta}</div>}<div className="xm-row-actions">{actions}</div></div>;
}
