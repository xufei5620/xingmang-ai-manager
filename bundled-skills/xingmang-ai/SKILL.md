...
description: >-
  ...
  默认走分组「图片模型-中转/订阅」的 Key。当用户说生图、画一张、生成图片、改图、星芒AI、星芒生图时使用。
...
调用星芒中转出图。Key 由**星芒AI管理工具**在登录后自动写入本目录 `config.json`，必须属于分组 `图片模型-中转/订阅`（别名 `生图分组`）。编程分组的 Key 调生图会 503。
...
没有 `config.json` 或里面没有 Key：提醒用户打开星芒AI管理工具并登录。工具会检测图片分组，没有 Key 就创建一个再写进配置。不要向用户要环境变量，也不要去翻 CLI 配置或加密会话。
