...
function ChatScope({ api, scope, active }: { api: ChatApi; scope: string; active: boolean }) {
  const chat = useChatController(api, scope)
      ...
      <div className="chat-messages" ref={list} aria-label="消息记录" onScroll={(event) => { if (!active) return; const element = event.currentTarget; stickToBottom.current = element.scrollHeight - element.scrollTop - element.clientHeight < 80; scrolls.current.set(conversation.id, element.scrollTop) }}>
        ...
        {conversation.messages.map((message) => <div key={message.id} className="chat-message" data-role={message.role} data-status={message.status} data-testid={`chat-message-${message.id}`}>
          ...
          <div className="chat-message-main">
            ...
            <div className="chat-message-tools">{message.content && <Button size="xs" icon={Copy} variant="ghost" aria-label="复制内容" title="复制内容" onClick={() => void copyText(message.content)} />}{message.role === 'assistant' ? <Button size="xs" icon={RefreshCw} variant="ghost" aria-label="重新生成" title="重新生成" disabled={pending} onClick={() => setConfirmation({ kind: 'retry', id: message.id, mayStillComplete: message.mayStillComplete })} testId={`chat-message-retry-${message.id}`} /> : <Button size="xs" icon={Pencil} variant="ghost" aria-label="编辑消息" title="编辑消息" disabled={pending} onClick={() => setEditing({ id: message.id, text: message.content })} testId={`chat-message-edit-${message.id}`} />}<Button size="xs" icon={Trash2} variant="ghost" aria-label="删除消息" title="删除消息" disabled={pending} onClick={() => setConfirmation({ kind: 'delete-message', id: message.id })} testId={`chat-message-delete-${message.id}`} /></div>
          </div>
        </div>)}
      </div>
      <div className="chat-composer">
        <div className="chat-compose-options"><Segment options={[{ value: 'text', label: '文本对话' }, { value: 'image', label: '生成图片' }]} value={conversation.settings.mode} onChange={onModeChange} testId="chat-mode" /><Select aria-label="分组" title={selectedGroup?.description || selectedGroup?.name} options={chat.groups.length ? chat.groups.map((group) => ({ value: group.name, label: group.name })) : [{ value: '', label: chat.groupLoading ? '正在读取分组' : '暂无可用分组' }]} value={conversation.settings.group} onChange={(event) => chat.selectGroup(event.target.value)} disabled={chat.groupLoading || !chat.groups.length} testId="chat-group" /><div className="chat-model-select"><BrandIcon model={conversation.settings.model} size={16} /><Select aria-label="模型" value={conversation.settings.model} options={[...(!conversation.settings.model || selectedModelMissing ? [{ value: conversation.settings.model, label: preparation?.phase === 'loading' ? '正在准备模型' : '请选择模型' }] : []), ...models.map((model) => ({ value: model, label: model }))]} onChange={(event) => { chat.selectModel(event.target.value); setCustomSize(false) }} disabled={preparation?.phase !== 'ready' || !models.length} testId="chat-model" /></div><Button icon={RefreshCw} variant="ghost" size="sm" aria-label="重新准备分组" title="重新准备分组" loading={preparation?.phase === 'loading'} disabled={!conversation.settings.group} onClick={() => void chat.prepareGroup(conversation.settings.group)} testId="chat-group-prepare" />{conversation.settings.mode === 'text' && <span className="chat-cost-hint">自动 · 费用不可预测</span>}</div>
        {preparation?.phase === 'error' && <p className="chat-message-error" role="alert">{preparation.error}</p>}{preparation?.phase === 'ready' && models.length === 0 && <p className="chat-hint">这个分组暂未提供{conversation.settings.mode === 'image' ? '图片' : '聊天'}模型，请切换分组。</p>}
