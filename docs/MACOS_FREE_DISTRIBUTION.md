...
首次发布时，在受控的发布 Mac 上生成一张有效期三年、加密保护并兼容 macOS Keychain 的 P12。证书名称限 1 至 64 个安全字符。P12 密码必须是 20 至 256 个可打印 ASCII 字符，并至少包含小写字母、大写字母、数字、符号中的三类；密码应由密码管理器生成并注入当前 shell，不要写进脚本或 shell 历史。生成器只能检查格式和字符类别，不能测量或保证密码的真实熵。
...
生成器签发的证书有效期为三年，且是不能签发下级证书的终端证书（`basicConstraints=critical,CA:FALSE`、`keyUsage=critical,digitalSignature`）。这两条都是发布门禁会校验的硬性条件：`npm run dist:mac:free` 的签名预检读取证书文本，发现它是 CA、`keyUsage` 里带 `Certificate Sign`／`CRL Sign`，或者 `basicConstraints` 不是 critical 的 `CA:FALSE`，都会直接失败。早期版本的生成器签发的是 20 年期、`CA:TRUE` 且带 `keyCertSign` 的证书；那种证书被发布 Mac 标记为代码签名可信之后，拿到 P12 的人就能在那台机器上继续签发链到可信锚的证书。手上还留着这种旧证书的，必须重新生成后再发布。

证书到期或需要提前轮换时：

1. 到期前至少一个月安排轮换。证书一旦过期就不能再签名，发布会卡住。
2. 按本节前面的步骤在发布 Mac 上生成新的 P12 和 `.cer`，并完成“信任”设置与指纹记录。旧的密钥材料在确认新证书可用之前不要销毁。
3. 用新的 `CSC_NAME` 与 `XINGMANG_MAC_SIGNING_SHA256` 跑一次 `npm run dist:mac:free`，确认预检、codesign 与产物校验全部通过。
4. 换证书必然中断 Squirrel.Mac 的更新连续性：老用户的自动更新会拒绝新版本（失败方向是安全的）。必须像首个自签版一样，让用户手动下载安装一次带新证书的 DMG，并在发版说明里写清楚这一次需要手动安装。
5. 轮换完成后销毁旧 P12 及其离线备份，避免旧密钥继续可用。
