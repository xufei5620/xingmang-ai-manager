...
- 渲染层「账号 origin → 站点 id」的映射收口到 `account-context.ts` 的 `siteIdForOrigin()`，`account-switch-sync.ts` 与 `App.tsx` 里各自抄写的三处字面量改为调用它；未知 origin 返回 `null`，调用方据此拒绝而不是静默当成历史站点（D-09，行为不变）。
