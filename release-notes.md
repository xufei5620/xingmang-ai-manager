...
- 以前接过别家中转站的工具，引导第 3 步不再卡在灰掉的「下一步」上：页面直接说「现在用的不是当前账号的 Key」，只放一颗「改用 你的账号名」，点一下就换成你自己账号的 Key 并自动进到下一步，改之前会先把原来的设置备份，在「备份」里能找回。没登录时这颗按钮写「登录后改用我的账号」，登录完会接着改好。
- 首页这类工具显示「不是当前账号的 Key」，行上直接有「改用 你的账号名」；同一个站上换了账号、工具里还是旧账号的 Key 时，显示「Key 可能不是当前账号的」，照常能用，也能一键改用。「…」菜单、CC Switch 那一行和改完的提示统一叫「改用」。
- 配置窗口里的「切换为星芒账号」「填写星芒密钥」「查看处理步骤」换成一颗「改用 你的账号名」，改完窗口不关，直接说「已改用，可以开始用了」；「自己填写密钥」收进「高级」。Key 不是当前账号的时候，密钥下拉框默认「自动准备」，不再默认留着那把用不了的 Key。
- 改用失败时按原因给出口：账号还没开通这个工具给「找客服」，余额不够给「去充值」，连自动恢复也没成功时给「去备份页」。
- 改用 Codex 时会说明 Codex CLI 和 Codex 桌面端是一起改的；CC Switch 还开着时，改完弹一个框提醒先退出它，不再是一闪而过的小提示。
