...
- 注册的用户名默认取邮箱 @ 前面的部分（按字符截到 20 位），用户改过就不再跟随，撞名时错误挂在用户名一格；邀请码默认折叠，带邀请码打开时展开。注册依据实际 `getAccountStatus` 开关校验，注册完成后单独调用登录；仅登录请求成功才回报已认证，失败则预填用户名回登录页。注册通道返回 `void`，不能作为登录成功证据。
- 登录和添加账号均可选“星芒账号 / 历史账号”，默认星芒账号且来源切换收起，点“用历史账号登录”才出现（同时选中历史账号）；登录、记住密码和找回密码绑定显式 `siteId`。来源切换清除旧密码和重置码，迟到的凭据读取不能覆盖新来源或用户已输入的内容。历史账号使用邮箱登录，显式选择失败不会尝试另一站。
- 星芒账号找回固定三步，重置请求发送 `email`、解析后的唯一 `token` 及独立来源参数。邮件链接必须属于所选来源。服务端返回的新密码默认遮盖；剪贴板失败保留可选中的文本。历史账号不调用客户端重置接口，提供对应官网恢复入口与客服说明；双重验证也给出对应官网入口，不宣称已在客户端完成验证。
