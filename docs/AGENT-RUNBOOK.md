...
| 看到根目录冒出 `\tmp\xingmang-managed-cli-*` 目录 | 泄漏源已根治（批次 0.2），正常不会再出现；若再见到即为新回归，删掉并排查 |
...
（2026-08-10 校准：本节曾列的历史起手顺序——#5、#2、#37 #38、#30 及其串行约束——已全部完成并删除。当前待办以 `docs/IMPROVEMENT-PLAN.md` 各批次状态横幅与 `HANDOFF.md` ②栏为准。）
