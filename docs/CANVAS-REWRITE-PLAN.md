...
| B2-5 参数单一真相源 | ○ | | **方案已改,不采用研究建议**。Langflow 的用户可配置 pin 机制适合开发者用户,不适合「想出张图」的普通客户;且两套控件读写同一 `node.data`,无数值分歧,害处是冗余不是正确性。改为:节点体保持唯一编辑位置,检查器参数区转只读摘要 + 定位动作,腾出空间给运行历史/费用/错误详情/血缘 |
| B2-6 pinned 持久化 | ⊘ | | 随 B2-5 方案调整取消。**批次 2 因此不再触碰持久化 schema**,先前声明的「唯一一处越界到保留层」消失 |
| B2-3 由最长选项反推最小宽度 | ○ | | B2-2 落地后截断已消失,此项改为「验证 + 设下限」 |
| B2-4 / B2-8 / B2-11 / B2-12 / B2-13 | ○ | | |
