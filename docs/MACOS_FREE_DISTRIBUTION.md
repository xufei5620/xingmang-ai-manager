...
2. 在“应用程序”里双击应用一次，提示框里选“完成”（不要选“移到废纸篓”）。
3. 进入“系统设置 > 隐私与安全性”，在“安全性”一段该应用的提示旁选择“仍要打开”，再在确认框里选“仍要打开”并输入登录密码。

macOS 15 起苹果去掉了“按住 Control 点按 / 右键点按 → 打开”这条放行路，右键打开和双击一样只给“完成 / 移到废纸篓”，所以第 3 步是所有受支持版本（macOS 13 起）共用的主路。“仍要打开”只在刚被拦下后出现一段时间，没看到就回到第 2 步再双击一次。macOS 14 及更早的系统仍可用右键点按 → “打开”。
...
1. 到期前至少一个月安排。证书一旦过期就不能再签名，发布会卡住。因私钥泄露而轮换时立即执行。
...
4. 像首个自签版一样，让用户手动下载安装一次带新证书的 DMG，并在发版说明里写清楚这一次需要手动安装。
