...
function Fixture() {
  ...
  const [tools, setTools] = useState<GuideToolState[]>(['claude', 'codex', 'codexDesktop', 'gemini', 'grok'].map((id) => ({ id: id as GuideToolState['id'], installed: query.has('installed'), configured: query.has('connected'), source: query.has('unknown') ? 'unknown' : query.has('official') ? 'official' : 'account', runtimeReady: query.has('runtime'), pythonReady: query.has('python'), officialLoginRequired: query.has('officialLoginRequired'), installMode: 'managed' })))
