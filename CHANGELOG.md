...
> **本文件已封存，只保留 0.1.0 ~ 0.1.12（2026-07-24 ~ 2026-08-02）的历史条目。**
> 0.1.20 之后的发版记录在 [`release-notes.md`](release-notes.md) —— 那份文件由发布流程随每次发版更新，
> 是唯一在维护的发版日志（当前最新 0.2.5，2026-09-16）。新条目请写在那里，不要再往本文件追加。
>
> 已知缺口：**0.1.13 ~ 0.1.19** 和 **0.1.29** 两处在两份文件里都没有条目，属历史遗留，不再回补。
