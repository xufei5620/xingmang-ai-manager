...
- AI 工作台聊天用的密钥如果设了额度上限、又用完了，聊天会停下来，说一句「聊天用的密钥额度用完了」，旁边有「去调额度」按钮直接打开「账号」页「密钥」。以前会悄悄换一把不限额的新密钥接着用。
...
- `chat-credential-coordinator.ts`：模型查询失败带 Key 额度字样，或 401 且按缓存 Key id 在密钥列表里查到封顶且用完时，抛 `ChatKeyQuotaExhaustedError`，不删缓存、不再签新 Key。`ai-chat-service.ts` 新增错误码 `key-quota-exhausted`；渲染层 `chatErrorMessage` 认这个码和这句话，`ChatPage` 在旁边给「去调额度」按钮（`onOpenKeys`，App 里导航到账号页密钥）。
