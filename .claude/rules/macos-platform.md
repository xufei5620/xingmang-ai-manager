...
> 从 `CLAUDE.md` 第 5 节 T5 搬出（2026-09-18）。行号引用已换成符号名，其余内容未改。这是按路径加载的规则：只有改到上面列出的文件时才进入上下文。
...
| `isTrustedHighIntegrityExecutable` | 绝对路径 + 上面那条。管住 `diagnostics.ts` 里两处可执行文件解析 |
