...
const sessionSummaries = [
  {
    ...
    messageCount: 42,
    sourcePath: `${workspace}/.codex/sessions/01JCODEXPREVIEW0001.jsonl`,
    detailAvailable: true,
  },
  {
    id: 'claude-preview-1',
    provider: 'claude' as const,
    nativeId: '01JCLAUDEPREVIEW001',
    title: '梳理账号中心的接口契约',
    cwd: workspace,
    ...
    messageCount: 18,
    sourcePath: `${workspace}/.claude/projects/demo/01JCLAUDEPREVIEW001.jsonl`,
    detailAvailable: true,
  },
  {
    id: 'gemini-preview-1',
    provider: 'gemini' as const,
    nativeId: '01JGEMINIPREVIEW001',
    title: '整理无限画布的节点吸附规则',
    cwd: workspace,
    model: 'gemini-3-pro',
    archived: true,
    ...
    messageCount: 7,
    sourcePath: `${workspace}/.gemini/tmp/01JGEMINIPREVIEW001.json`,
    detailAvailable: true,
  },
]
...
const sessionPage = {
  ...
  stats: {
    total: sessionSummaries.length,
    byProvider: { codex: 1, claude: 1, gemini: 1, grok: 0 },
  },
...
const handlers: Partial<Record<keyof XingmangApi, unknown>> = {
  ...
  listProviderSessions: async () => sessionPage,
  listSessions: async () => sessionPage,
  openCanvasWindow: async () => undefined,
  listAiChatGroups: async () => usableGroups,
  prepareAiChatGroup: async (group: string) => ({
    group,
    models: ['gpt-5-codex', 'claude-sonnet-4.6', 'gemini-3-pro'],
    keyCreated: false,
  }),
  startAiChat: async () => ({ requestId: `preview-${Date.now()}`, accepted: true as const }),
  cancelAiChat: async () => ({ canceled: true, mayStillComplete: false }),
  generateAiImage: async () => [],
  copyAiChatAsset: async () => undefined,
  saveAiChatAsset: async () => ({ saved: false }),
  showAiChatAssetMenu: async () => undefined,
  listBackups: async () => [],
