...
**测试基线**：Windows 上有 0~9 个环境相关的已知失败（符号链接权限、Defender 超时），不是回归。动手前先在干净基线上跑一遍记下失败数，改完对比，**不要引入新失败**。各平台明细、复核命令与容器提示见 `docs/TEST-BASELINE.md`。

**跨平台**：改跨平台代码前先读 `electron/platform-capabilities.ts`，它是判断"当前平台支持什么"的单一入口；macOS 相关模块清单见 `docs/MODULE-MAP.md`。

---

## 3. 模块地图

完整模块地图（主进程各模块职责与行数、渲染进程结构）在 `docs/MODULE-MAP.md`，按需再读。改代码前必须知道的只有三条：

- `ipc-contract.ts` 是唯一的跨进程类型真相源；`preload.ts` 里的通道表是 sandbox 约束下的有意重复（见 I7、T1）。
- 本项目的复杂度不在业务，在 `command-runner.ts` / `windows-elevation.ts` / `trusted-temp.ts` 这套提权、可信路径与原子写入的边界上（见第 4 节）。
- 画布（源码 `canvas-v2/`，产物 `dist-canvas/` 不入 git）是全项目唯一运行第三方前端代码的地方，全部 AI 调用都在主进程完成（见 I15、T13）。
...
Windows 问「低于 Administrator 的主体能不能写这里」，因为那边程序可能持有提权令牌；macOS 上本程序从不提权，边界换成「除 root 与当前用户之外的主体能否改动它解析后的目标」。三个函数在 macOS 上的语义、两个最容易误读的点、刻意不做的事与仍然欠着的事，见 `.claude/rules/macos-platform.md`——改到 `electron/macos-*` / `darwin-*` / `platform-capabilities.ts` 时会自动加载。
...
- `docs/MODULE-MAP.md` — 模块地图：主进程各模块职责与渲染进程结构。**找文件先看这里**
- `docs/TEST-BASELINE.md` — 各平台测试基线、已知环境相关失败与复核命令
- `.claude/rules/macos-platform.md` — macOS 路径信任的完整推导（改相关文件时自动加载）
