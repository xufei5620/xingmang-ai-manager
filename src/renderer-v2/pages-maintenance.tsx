    ...
    steps: [
      {
        title: '连上和断开',
        detail:
          '在「游戏加速」点「开始加速」，页面显示出线路名称和延迟就算连上了，再点一次「停止加速」断开。不选线路就是「智能分配」，连接时自动测速挑最快的一条；也可以点「选择加速线路」自己指定，连接期间不能换线，停下来才能改。「TUN 模式」那一栏目前写着「暂未开放」，不用管它。',
        action: '打开游戏加速',
        page: 'acceleration',
      },
      {
        title: '免费时长怎么算',
        detail:
          '每个账号在本机累计 20 分钟，不每日重置，只有真的连上才计时，停止后剩下的留着下次用。用完之后按钮会变成「免费体验已用完」，旁边的「帮助与客服」可以联系我们。手里有加速口令的，按 Ctrl / Command + K 打开命令面板，把口令整串输进去就能加 10 分钟，同一个账号在本机只能领一次。',
        action: '打开游戏加速',
        page: 'acceleration',
      },
      {
