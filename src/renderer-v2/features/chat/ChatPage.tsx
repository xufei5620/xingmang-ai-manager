...
import { chatErrorMessage, filterConversations, isChatKeyQuotaError, isGenerating, shouldSendOnEnter, type ChatMessage, type ChatMode } from './state'
import { ParametersPanel } from './ParametersPanel'
import { useChatController } from './useChatController'
import './chat.css'

export interface ChatPageProps { bridge: XingmangApi; accountScope: string; active?: boolean; onOpenKeys?: () => void }
type Confirmation = { kind: 'retry' | 'delete-message' | 'delete-conversation' | 'clear' | 'stop'; id?: string; mayStillComplete?: boolean } | null

export function ChatPage({ bridge, accountScope, active = true, onOpenKeys }: ChatPageProps) {
  const api = useMemo(() => createChatApi(bridge), [bridge])
  return <ChatScope key={accountScope} api={api} scope={accountScope} active={active} onOpenKeys={onOpenKeys} />
}

function ChatScope({ api, scope, active, onOpenKeys }: { api: ChatApi; scope: string; active: boolean; onOpenKeys?: () => void }) {
  ...
  return <section ref={surface} className="chat-page" hidden={!active} data-testid="page-chat" data-page-id="ai-chat" data-account-scope={scope} data-busy={running} data-unsaved={unsaved} onKeyDownCapture={(event) => { if (!active) { event.preventDefault(); event.stopPropagation() } }} onScrollCapture={(event) => { if (active && event.target instanceof HTMLElement && event.target.classList.contains('chat-conversation-list')) navigationScroll.current = event.target.scrollTop }}>
    ...
    <div className="chat-conversation">
      ...
      <div className="chat-messages" ref={list} aria-label="消息记录" onScroll={(event) => { if (!active) return; const element = event.currentTarget; stickToBottom.current = element.scrollHeight - element.scrollTop - element.clientHeight < 80; scrolls.current.set(conversation.id, element.scrollTop) }}>
        ...
        {conversation.messages.map((message) => <div key={message.id} className="chat-message" data-role={message.role} data-status={message.status} data-testid={`chat-message-${message.id}`}>
          ...
          <div className="chat-message-main">
            ...
            {message.status === 'error' && <p className="chat-message-error" role="alert">{message.error}{onOpenKeys && isChatKeyQuotaError(message.error) && <Button size="xs" variant="ghost" onClick={onOpenKeys} testId="chat-key-quota-open">去调额度</Button>}</p>}{message.status === 'canceled' && <p className="chat-message-note">{message.mayStillComplete ? '已停止等待，服务端仍可能处理并计费' : '已停止生成，保留已返回的内容'}</p>}
      ...
      <div className="chat-composer">
        ...
        {selectedGroupMissing && <p className="chat-message-error" role="alert" data-testid="chat-group-unavailable">当前分组已不可用，请选择可用分组。已保存的消息和草稿继续保留。</p>}{preparation?.phase === 'error' && <p className="chat-message-error" role="alert">{preparation.error}{onOpenKeys && isChatKeyQuotaError(preparation.error) && <Button size="xs" variant="ghost" onClick={onOpenKeys} testId="chat-key-quota-open">去调额度</Button>}</p>}{preparation?.phase === 'ready' && models.length === 0 && <p className="chat-hint">这个分组暂未提供{conversation.settings.mode === 'image' ? '图片' : '聊天'}模型，请切换分组。</p>}
