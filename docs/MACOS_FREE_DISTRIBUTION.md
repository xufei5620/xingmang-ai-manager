...
质量检查工作流不读取生产凭据：它只创建一次性自签身份和隔离临时钥匙串来执行真实打包门禁。签名期间，用户钥匙串搜索列表只包含该临时钥匙串；管理员域的临时代码签名信任只允许通过 GitHub 托管 macOS runner 的无交互 `sudo -n` 写入，runner 如果不能无密码执行就会立即失败，不会等待密码框。完成或失败后，流程都会恢复原钥匙串搜索列表，并删除临时信任、钥匙串、证书与构建产物；这些产物不能用于发布。
