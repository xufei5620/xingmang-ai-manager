...
try {
  ...
  await createDialog.getByRole('textbox', { name: '项目名称' }).fill('视觉回归项目')
  await createDialog.getByRole('button', { name: '选择文件夹并创建' }).click()
  ...
  assert.equal(await mediaConfiguration.locator('select[aria-label$="分组选择"]').count(), 3, '生成配置应提供生图、视频与文字分组')
  assert.equal(await mediaConfiguration.locator('select[aria-label$="默认模型"]').count(), 3, '每个生成分组都应提供默认模型')
  ...
  await page.locator('.node-library-item').filter({ hasText: '视频抽帧' }).click()
  const runnableNode = page.locator('.react-flow__node-frame-extract').last()
  await runnableNode.waitFor({ state: 'visible' })
  await runnableNode.locator('header').click()
  ...
  await page.keyboard.press('Delete')
  await runnableNode.waitFor({ state: 'detached' })
  ...
  const imageInputNode = page.locator('.react-flow__node-image-input').last()
  await imageInputNode.waitFor({ state: 'visible' })
  await imageInputNode.getByRole('button', { name: '从文件选择', exact: true }).waitFor({ state: 'visible' })
  ...
  await fixtureAsset.evaluate((element) => element.scrollIntoView({ block: 'center', inline: 'center' }))
  await fixtureAsset.hover()
  await page.waitForTimeout(180)
  const favoriteButton = fixtureAsset.getByRole('button', { name: /收藏：visual-fixture\.png/ })
  ...
  await page.getByRole('region', { name: '素材详情：visual-fixture.png' }).getByRole('button', { name: '放大预览：visual-fixture.png' }).click()
  ...
  assert.deepEqual(qualityOptions, ['标准', '高', '极高'], '图像画质选项应只显示档位')
  ...
  await videoTarget.locator('.wf-node').waitFor({ state: 'visible' })
  await page.waitForFunction((nodeId) => document.querySelector(`.react-flow__node[data-id="${nodeId}"]`)?.classList.contains('selected'), videoTargetId)
  const videoComposer = page.locator(`.wf-composer[data-id="${videoTargetId}"]`)
  await videoComposer.waitFor({ state: 'visible' })
  assert.equal(await videoComposer.locator('select[aria-label="视频模型"]').inputValue(), 'minimax-h3-base', 'MiniMax H3 Base 未成为默认视频模型')
  ...
  await videoComposer.locator('select[aria-label="视频模型"]').selectOption('grok-imagine-video')
  ...
  await page.getByRole('button', { name: '收起创作库' }).click()
  await page.locator('.node-library.is-collapsed').waitFor({ state: 'visible' })
  ...
  const emptyPointBeforeConnections = await findEmptyPanePoint(page)
  await page.mouse.click(emptyPointBeforeConnections.x, emptyPointBeforeConnections.y)
  await page.locator('.wf-composer').waitFor({ state: 'hidden' })
  ...
  for (const [index, node] of connectionNodes.entries()) {
    await node.click({ position: { x: 42, y: 22 }, modifiers: index === 0 ? [] : multiSelectModifiers, force: true })
  }
  ...
  let emptySelectionPoint = await findEmptyPanePoint(page)
  await page.mouse.click(emptySelectionPoint.x, emptySelectionPoint.y)
  await page.locator('.wf-composer').waitFor({ state: 'hidden' })
  await videoTarget.evaluate((element) => element.click())
  await page.getByLabel('画布导航').getByRole('button', { name: '聚焦选中节点' }).click({ force: true })
  await page.waitForTimeout(360)
  const connectedComposer = page.locator(`.wf-composer[data-id="${videoTargetId}"]`)
  ...
  emptySelectionPoint = await findEmptyPanePoint(page)
  await page.mouse.click(emptySelectionPoint.x, emptySelectionPoint.y)
  await page.locator('.wf-composer').waitFor({ state: 'hidden' })
  await imageEditTarget.evaluate((element) => element.click())
  const imageRefsComposer = page.locator(`.wf-composer[data-id="${imageEditId}"]`)
  ...
  await page.locator('.wf-composer').waitFor({ state: 'hidden' })
  await videoTarget.evaluate((element) => element.click())
  const videoPrompt = connectedComposer.locator('textarea')
  ...
  await page.keyboard.press('Control+K')
  let reopenedNodeSearch = page.getByRole('dialog', { name: '快速创建' })
  await reopenedNodeSearch.getByRole('combobox', { name: '搜索现有节点、新节点或命令' }).fill(imageGenerateNodeId)
  await page.keyboard.press('Enter')
  await reopenedNodeSearch.waitFor({ state: 'hidden' })
  const reopenedImageComposer = page.locator(`.wf-composer[data-id="${imageGenerateNodeId}"]`)
  await reopenedImageComposer.locator('select[aria-label="生成尺寸"]').waitFor({ state: 'visible' })
  assert.equal(await reopenedImageComposer.locator('select[aria-label="生成尺寸"]').inputValue(), '1280x720', '重新打开项目后图片比例没有保存')
  await page.keyboard.press('Control+K')
  reopenedNodeSearch = page.getByRole('dialog', { name: '快速创建' })
  await reopenedNodeSearch.getByRole('combobox', { name: '搜索现有节点、新节点或命令' }).fill(videoTargetId)
  await page.keyboard.press('Enter')
  await reopenedNodeSearch.waitFor({ state: 'hidden' })
  const reopenedVideoComposer = page.locator(`.wf-composer[data-id="${videoTargetId}"]`)
  await reopenedVideoComposer.locator('select[aria-label="视频模型"]').waitFor({ state: 'visible' })
  assert.equal(await reopenedVideoComposer.locator('select[aria-label="视频模型"]').inputValue(), 'minimax-h3-base', '重新打开项目后 MiniMax 模型没有保存')
  assert.equal(await reopenedVideoComposer.locator('select[aria-label="MiniMax 生成模式"]').inputValue(), 'ref2va', '重新打开项目后 MiniMax 模式没有保存')
  assert.equal(await reopenedVideoComposer.locator('select[aria-label="MiniMax 视频分辨率"]').inputValue(), '480p', '重新打开项目后 MiniMax 分辨率没有保存')
  assert.equal(await reopenedVideoComposer.locator('select[aria-label="MiniMax 视频比例"]').inputValue(), '9:16', '重新打开项目后 MiniMax 比例没有保存')
  assert.equal(await reopenedVideoComposer.getByText('AI 优化 H3 提示词', { exact: true }).locator('..').locator('input').evaluate((input) => input.checked), true, '重新打开项目后提示词优化开关没有保存')
