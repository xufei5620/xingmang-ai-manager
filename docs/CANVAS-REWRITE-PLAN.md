...
| B3-13 reroute 节点 | ⊘ 待决策 | | **不是渲染层能独立完成的**。中转节点必须在运行时把上游数据原样透传给下游,而执行在主进程(`canvas-run-engine.ts`),且要新增节点类型进持久化 schema。详见下方「需要拍板的两项」 |
| B3-16 旁路(bypass)语义 | ⊘ 待决策 | | 核实:现有 `disabled` 是**跳过**——`canvas-run-engine.ts:305/562` 把节点标记 skipped,并经 `upstreamSkipped` 把下游一并跳过,**没有透传**。真正的 bypass 要改主进程执行语义 + 持久化字段,且会改变用户可见的运行结果。详见下方「需要拍板的两项」 |

### 需要拍板的两项(B3-13 / B3-16)

两项都越出了「画布渲染层」的边界,且会改变用户可见的运行与计费结果,所以停下来问:

**B3-16 旁路**。现在只有「跳过运行」一种状态:节点不跑,下游也全部跳过。
ComfyUI 那种 bypass 是「节点不跑,但把输入原样透传给下游」,用于临时摘掉链路
中的一步做 A/B(比如 提示词 → 放大 → 输出,想看不放大的效果)。要做需要:

1. 持久化新增一个节点状态(是保留 `disabled` 再加 `bypassed` 两个独立状态,
   还是合成一个三态?)
2. `canvas-run-engine.ts` 支持按媒体类型匹配的透传,类型对不上时如何处理
3. 下游从「被跳过」变成「正常成功」——这条会改变用户看到的运行结果与费用预估

**B3-13 reroute 节点**。纯视觉的中转点,用来把连线绕开节点堆。同样要新增节点
类型进 schema,并让执行引擎把它当透传处理。技术上是 B3-16 透传能力的一个特例,
**如果 B3-16 做了,这项成本会低很多**;单独做则要为一个纯视觉需求改执行引擎。

建议:两项一起做或一起不做。若要做,B3-16 的第 1 点(状态如何建模)需要老板定。
