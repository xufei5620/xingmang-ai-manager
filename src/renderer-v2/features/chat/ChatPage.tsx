...
import { chatErrorAction, chatErrorMessage, filterConversations, isConversationTooLongMessage, isGenerating, shouldSendOnEnter, shouldShowLengthNotice, type ChatMessage, type ChatMode } from './state'
...
function ChatView({ api, scope, active, onOpenAccount, history }: ChatScopeProps & { history: LoadedChatHistory }) {
  ...
  return <section ref={surface} className="chat-page" hidden={!active} data-testid="page-chat" data-page-id="ai-chat" data-account-scope={scope} data-busy={running} data-unsaved={unsaved} onKeyDownCapture={(event) => { if (!active) { event.preventDefault(); event.stopPropagation() } }} onScrollCapture={(event) => { if (active && event.target instanceof HTMLElement && event.target.classList.contains('chat-conversation-list')) navigationScroll.current = event.target.scrollTop }}>
    ...
    <div className="chat-conversation">
      ...
      {(chat.groupError || chat.error || chat.notice || chat.storageError || preparation?.warning) && <div className="chat-banner" data-tone={chat.groupError || chat.error || chat.storageError ? 'bad' : 'neutral'} role={chat.groupError || chat.error || chat.storageError ? 'alert' : 'status'}><span>{chat.storageError || chat.groupError || chat.error || chat.notice || preparation?.warning}</span>{chat.groupError && <Button size="xs" icon={RefreshCw} onClick={() => void chat.refreshGroups()} testId="chat-groups-retry">重试</Button>}{!chat.groupError && !chat.storageError && isConversationTooLongMessage(chat.error) && <Button size="xs" variant="primary" icon={Plus} onClick={chat.continueInNew} testId="chat-continue-new">带着这句话开新对话</Button>}{!chat.groupError && !chat.storageError && !preparation?.warning && <Button variant="ghost" size="xs" icon={X} aria-label="关闭提示" title="关闭提示" onClick={() => { chat.setError(''); chat.setNotice('') }} />}</div>}
