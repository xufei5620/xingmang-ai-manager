...
async function open(query = '', stored = {}) {
  ...
  // reports it as the composer failing to appear, which reads like a broken
...
test('parameters and system prompt are applied to the request and text copy has a manual fallback', async () => {
  ...
  try {
    ...
    await emit(page, { type: 'complete', requestId: request.requestId })
    await page.locator('.chat-message[data-role=assistant]').getByRole('button', { name: '复制内容', exact: true }).click()
...
test('focus, reactivation and visible foreground polling refresh groups without repeating preparation', async () => {
  ...
  try {
    ...
    await page.getByTestId('page-chat').waitFor({ state: 'hidden' })
...
test('hidden chat keeps its request and drafts while exposing native close protection flags', async () => {
  ...
  try {
    ...
    await page.getByTestId('page-chat').waitFor({ state: 'visible' })
