...
**基线失败集合(Windows 本机,环境相关,非回归)**:9 项 = 4 项符号链接 `EPERM`(需开发者模式)+ 5 项 Defender 引起的 5s 超时。任何新增失败都按回归处理。
...
| B1-9 自绘 Controls | ○ | | |
| B1-10 MiniMap 状态着色 | ● | (见下) | `nodes/minimap-node-color.ts` 纯函数 + 4 项测试。**按优先级而非查表**:失败 > 运行中 > 排队 > 待更新 > 成功 > 空闲,确保失败永不被陈旧的成功状态掩盖 |
| B1-11 双层 Background | ● | (见下) | 16px 点阵次栅格 + 96px 线条主栅格,让对齐与吸附有视觉依据。四视口亮度升 2-3 点(远低于 90 阈值),`veryLightRatio` 不变 |
| B1-12 响应式覆盖式抽屉 | ○ | | |
| B1-13 面板状态存比例 | ○ | | |
| B1-14 ariaLabelConfig 中文化 | ● | (见下) | `aria-labels.ts` 覆盖全部 12 条内置英文文案(节点/连线的键盘说明、实时播报、控件、缩略图、端口)。smoke 端到端断言「放大」存在且 "Zoom In" 不存在,防止键名写错时静默回退 |
| B1-15 死 CSS 清理 | ○ | | |
