...
export const tutorialTopics = [
  ...
  },
  {
    id: 'sessions',
    title: '记录',
    lead: '四个命令行工具在这台电脑上留下的对话都在这一页，可以接着聊、查看、导出和整理。',
    steps: [
      {
        title: '先看这一页有什么',
        detail:
          '左侧「记录」里是 Claude Code、Codex CLI、Gemini CLI、Grok CLI 在本机留下的对话，每一行写着标题、工作文件夹、模型、消息条数和时间。顶部可以按工具筛选，也可以搜标题、文件夹或模型。这些内容一直只存在这台电脑上。',
        action: '打开记录',
        page: 'sessions',
      },
      {
        title: '接着上次的对话',
        detail:
          '想继续昨天那件事，点这一行的「接着聊」：软件会在这条记录的文件夹里打开对应的工具，并接上这个文件夹里最近的一次对话，不用自己开终端敲参数。命令行工具只能按文件夹找回最近一条，所以同一个文件夹里更早的记录、已归档的记录、没记下文件夹的记录都不给这颗按钮。首页「最近」卡上的那几行也有同一颗按钮。',
        action: '打开记录',
        page: 'sessions',
      },
      {
        title: '查看、导出、归档',
        detail:
          '点「查看记录」在右边翻这次对话的往来内容，底部「导出 Markdown」会存成一个文件并告诉你存到了哪里。不想让某条再占着列表就点「归档记录」，之后还能「恢复记录」。归档会把它移出工具自己的目录，所以归档之后这条记录不能再「接着聊」。',
        action: '打开记录',
        page: 'sessions',
      },
      {
        title: '两种常见情况',
        detail:
          '「接着聊」按不动、旁边标着「文件夹已不存在」，是这条记录的文件夹被删掉或搬走了，工具没法再按文件夹找回它；这类记录仍然可以查看和导出。记录能留多久由工具自己管：由本软件写配置的 Claude Code 与 Gemini CLI，保留期已经从一个月放长到一年，你自己设过保留期的照你设的来。',
        action: '打开记录',
        page: 'sessions',
      },
    ],
  ...
  },
  {
    id: 'acceleration',
    title: '游戏加速',
    lead: '加速只接管这台电脑的系统代理设置；登录后每个账号有 20 分钟免费时长，随用随停。',
    steps: [
      {
        title: '连上和断开',
        detail:
          '在「游戏加速」点「开始加速」，页面显示出线路名称和延迟就算连上了，再点一次「停止加速」断开。主窗口缩到托盘里的时候不用再叫出来：托盘图标右键菜单上有一行加速状态和剩余时长，下面就是「连接加速 / 断开加速」，用不了时那一项是灰的、状态行写着为什么。不选线路就是「智能分配」，连接时自动测速挑最快的一条；也可以点「选择加速线路」自己指定，连接期间不能换线，停下来才能改。「TUN 模式」那一栏目前写着「暂未开放」，不用管它。',
        action: '打开游戏加速',
        page: 'acceleration',
      },
      {
        title: '免费时长怎么算',
        detail:
          '每个账号在本机累计 20 分钟，不每日重置，只有真的连上才计时，停止后剩下的留着下次用。「本次连接」和「累计使用」两个数字就在页面下方，随时能看。用完之后按钮会变成「免费体验已用完」，旁边的「帮助与客服」可以联系我们。',
        action: '打开游戏加速',
        page: 'acceleration',
      },
      {
        title: '有时候它会自己连',
        detail:
          '安装或更新命令行工具、下载 Node.js 时，软件会自动启用一条只用于下载的临时线路：不改电脑的网络设置，下载完自动关闭，加速页也不会因此显示为已连接，这段时间不计入免费时长。从软件里打开 Codex 桌面端时，如果加速还没连上会先替你连一次再打开——这一次是正常加速，会计时，而且不会自动断开，用完请回到这一页自己停止。已经在用加速的，软件一律不动你的线路。',
        action: '打开游戏加速',
        page: 'acceleration',
      },
      {
        title: '连不上怎么办',
        detail:
          '点「开始加速」后先提示检测到其他代理或 VPN 的，把它们关掉再点，或者直接选「仍然连接」。看到「加速服务暂不可用」这一类提示，先照它那句话做——例如完全退出软件（含托盘图标）后重开、检查磁盘剩余空间、请电脑管理员放行本软件；提示旁边的「查看日志」会跳到反馈页的运行日志，里面记着这次卡在了哪一步，「重新检查」可以再读一次状态。',
        action: '打开游戏加速',
        page: 'acceleration',
      },
    ],
    ...
    steps: [
      {
        title: '先知道为什么要自己装',
        detail:
          'Windows 上这四个桌面端星芒可以替你装，macOS 上没有这条路：这几家官方在 Mac 上只提供自己下载的安装包。首页上它们那一行写着「暂不支持」或点了就把你带到这里，说的都是这件事，不是它们用不了。整个过程不用管理员密码，也不会动你已经装好的命令行工具。',
        action: '返回首页',
        page: 'home',
      },
      {
  ...
  },
  {
    id: 'messages',
    title: '看到这句提示怎么办',
    lead: '把界面上会出现的提示按地方归好，每一句写清它在说什么、下一步做什么。',
    steps: [
      {
        title: '装不上、更新不了',
        detail:
          '「磁盘空间不够」是安装目录所在的磁盘满了，清理后重试，对话框里的「复制路径」能把这个目录复制走。「写不进安装目录」：本软件按普通权限运行、不会提权，请检查这个目录的写入权限，或看看是不是被杀毒软件拦了。「安装被杀毒软件拦住了」：去杀毒软件的隔离记录里恢复，并把这个目录加进信任区。「工具正在运行」是文件被占用，先关掉正在使用这个工具的窗口再重试。「下载超时」时已装好的工具不受影响，点「换官方源重试」即可。',
        action: '打开安装卸载',
        page: 'maintenance',
      },
      {
        title: '连不上、登录不了',
        detail:
          '「连接被证书拦截」是网络里有设备替换了证书（常见于公司网络或安全软件），换个网络（例如手机热点）或关掉拦截再试。「证书日期对不上」多半是这台电脑的时间不准，把系统时间设成自动同步、确认时区之后再试。「连不上星芒服务器」「请求太频繁」时，已经装好的工具照常能用，过一会儿重试。「登录已过期」不影响工具里已写入的 Key，只是余额和用量不再更新，重新登录即可；「Key 失效」点「一键修复」按当前账号重写一次，「余额已用完」充值到账后立即恢复。',
        action: '打开检查',
        page: 'health',
      },
      {
        title: '首页和打开文件夹时',
        detail:
          '工具行上的「配置被改过」，是这个工具的配置被别的程序或手工改动过，当前账号的 Key 可能已经不在里面了：点旁边的「重新写入 Key」按当前账号写回去，确实是自己改的就在「…」里选「就用现在这份」，以后不再提示。「检测失败」不等于没装，它会写明这次为什么没读出来。打开文件夹时提示「这个文件夹范围太大」，是你选了主目录、磁盘根目录、桌面、下载或文档，建议改选一个具体的项目文件夹；仍然打开也可以，只是这一次不会把它标成可信，也不会在里面生成项目说明文件。',
        action: '返回首页',
        page: 'home',
      },
      {
        title: '还是不知道怎么办',
        detail:
          '先到「检查」页跑一次，结果里的「去处理」会直接把你带到该改的那一页。提示上的「查看日志」按失败的来源落地：环境类问题去反馈页的运行日志，安装本身的失败去安装卸载页的安装日志卡。仍然解决不了，就在反馈页确认报告内容后复制或下载发给客服；报告里会附上你上次在检查页做的那次自检结果，不用再去截图。',
        action: '打开反馈',
        page: 'feedback',
      },
    ],
