...
- 客户端：`updater.ts` 新增 `verifyPackageSignature`，在 SHA-512 对上之后对同一清单项验签，失败按下载失败处理（`UPDATE_SIGNATURE_MISSING` / `UPDATE_SIGNATURE_INVALID` / `UPDATE_SIGNATURE_UNCONFIGURED`）；`main.ts` 只在未签名通道（Windows）接上。验签模块 `electron/update-package-signature.ts`：公钥名单 `updateSigningPublicKeys` 可放多把以便换钥匙；缺签名、签名不对、没配公钥一律拒装，不会因为签名缺失放行。macOS 不签，Squirrel.Mac 已按钉住的发布证书验苹果签名。
