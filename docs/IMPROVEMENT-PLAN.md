...
### 3.4 更新签名发布者从裸 CN 收紧为完整 DN

> **代码已落地（2026-09-05 审计）**：运行时与发布产物校验均拒绝裸 CN 和部分 DN，要求至少包含 CN、O、C 且属性集合和值完全匹配；正式发布预检提前拒绝不完整配置，原 CN-only 告警分支已删除。正式发布前须将 `XINGMANG_SIGNING_PUBLISHER` 配置为实际证书完整 Subject（保留所有属性）。此次不更换签名证书或 CN，旧客户端仍按既有校验逻辑验证下一版安装包；证书轮换须另行规划。以下为原始风险与迁移分析，保留归档，不代表当前实现。
