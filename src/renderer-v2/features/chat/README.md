...
记录经主进程存到用户数据目录的 `chat-history/` 下（`electron/ai-chat-history-store.ts`），一个对话一个文件加一份索引，每次只重写改过的对话，不再受 localStorage 的 4 MB 限制；页面先读到记录再显示，避免空白状态抢先覆盖。旧版本留在 localStorage 的 `xingmang-ui-v2:chat:<scope>` 只读不写，新存储还没有这个账号的记录时读它，首次保存即完成迁移。记录不包含运行时素材 URL 和请求 ID。写盘前还会剥掉正文、思考、错误、标题、草稿、系统提示词和图片提示词里的凭据（`sk-`、`Bearer`、`api_key=`/`token=` 这类键值）与运行时专用负载（`blob:`、内联 `data:` base64），窗口里的内容不受影响；普通链接和正文照原样保存，本地记录对真实内容仍然无损。读取时只重建合法的本机素材协议地址，未完成消息降为已停止。无法读取的记录保留原始值，并持续说明当前会话不能保存，避免悄悄覆盖损坏数据。
